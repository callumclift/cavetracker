...
import 'completed_activity_log_club.dart';

class CompletedClubActivityLogsListPage extends StatefulWidget {


  CompletedClubActivityLogsListPage();

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _CompletedClubActivityLogsListPageState();
  }
}

class _CompletedClubActivityLogsListPageState extends State<CompletedClubActivityLogsListPage> {
  ...
  initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      context.read<ActivityLogModel>().getActivityLogs(true);
    });
  }


  void _viewActivityLog(int index){
    context.read<ActivityLogModel>().selectActivityLogClub(context.read<ActivityLogModel>().allActivityLogsClub[index]['document_id']);
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (BuildContext context) {
      return CompletedActivityLogClubPage();
    })).then((_) {
      context.read<ActivityLogModel>().selectActivityLogClub(null);
    });
  }
  ...
  void loadMore() async {
    ...
    await context.read<ActivityLogModel>().getMoreActivityLogs(true);
    setState(() {
      _loadingMore = false;
    });
  }
          ...
          InkWell(onTap: () => _viewActivityLog(index),
            child: ListTile(
              ...
              subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                GlobalFunctions.boldTitleText('Caver: ', activityLogs[index]['username'], context),
  ...
  Widget _buildPageContent(List<Map<String, dynamic>> activityLogs) {
    ...
    if (context.read<ActivityLogModel>().isLoading) {
      ...
      return RefreshIndicator(
          ...
          onRefresh: () => context.read<ActivityLogModel>().getActivityLogs(true));
    } else {
      return RefreshIndicator(
        color: darkBlue,
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<ActivityLogModel>(
      builder: (context, model, child) {
        List<Map<String, dynamic>> activityLogs = model.allActivityLogsClub;
        return Scaffold(
            appBar: AppBar(iconTheme: IconThemeData(color: darkBlue),
              backgroundColor: mintGreen,
              title: FittedBox(fit:BoxFit.fitWidth,
                  child: Text('Club Activity Logs', style: TextStyle(color: darkBlue, fontWeight: FontWeight.bold),)),),
            drawer: SideDrawer(),
