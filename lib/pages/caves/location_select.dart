...
class LocationSelect extends StatefulWidget {
  ...
  LocationSelect(this.updateLocation, this.currentLatitude, this.currentLongitude, this.cave);

  @override
  State<LocationSelect> createState() => LocationSelectState();
}
...
class LocationSelectState extends State<LocationSelect> {
  ...
  _setUpInitialCameraPosition() async {
    ...
    if (currentLocation.latitude != null &&
        currentLocation.longitude != null) hasCurrentLocation = true;
    if(widget.currentLatitude.isNotEmpty){
      _initialCameraPosition = CameraPosition(
        target: LatLng(double.parse(widget.currentLatitude), double.parse(widget.currentLongitude)),
        zoom: 15.4746,
      );
  ...
  _loadInitialMarker(){
    ...
    if(widget.currentLatitude.isNotEmpty && widget.currentLongitude.isNotEmpty){
      LatLng latLng = LatLng(double.parse(widget.currentLatitude), double.parse(widget.currentLongitude));
      setState(() {
        _markers.add(Marker(
  ...
  Widget build(BuildContext context) {
    return new Scaffold(backgroundColor: whiteGreen,
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text('Confirm Location', style: TextStyle(color: whiteGreen),),
      ),
    );
  }

  _confirmLocation(){
    if(selectedLat != null && selectedLong != null) widget.updateLocation(selectedLat, selectedLong, widget.cave);
